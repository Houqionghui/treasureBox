    ...
    $.fn.WheelSurf = function (options) {
        ...
        return {
            init: function (angelTo) {
                ...
                ctx.arc(0, 0, 121, 0, 2 * Math.PI)
                ctx.stroke();
                // 画里圆
                ctx.beginPath();
                ctx.lineWidth = 12;
                ctx.strokeStyle = _default.innerCircle.color;
                ctx.arc(0, 0, 109, 0, 2 * Math.PI)
                ctx.stroke();
                ...
                for (var i = 0; i < num; i++) {
                    ...
                    ctx.arc(0, 0, 52, startAngel, endAngel)
                    ctx.stroke();
                    startAngel = endAngel
                    endAngel += angel
                }
